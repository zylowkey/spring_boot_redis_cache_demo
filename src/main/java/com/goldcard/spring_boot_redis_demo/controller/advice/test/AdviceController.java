package com.goldcard.spring_boot_redis_demo.controller.advice.test;

import com.goldcard.spring_boot_redis_demo.exception.NotFoundException;
import org.springframework.stereotype.Controller;
import org.springframework.ui.ModelMap;
import org.springframework.web.bind.annotation.GetMapping;
import org.springframework.web.bind.annotation.RequestMapping;

import java.util.Date;

@Controller
@RequestMapping("/advice")
public class AdviceController {
    @GetMapping("/test")
    // 因为日期格式被控制器通知限定，所以无法再给出
    public String test(Date date, ModelMap modelMap) {
        System.out.println("=============="+date);
        // 从数据模型中获取数据
        System.out.println(modelMap.get("project_name"));
        // 打印日期参数
//        System.out.println(DateUtils.format(date, "yyyy-MM-dd"));
        // 抛出异常，这样流转到控制器异常通知
//        throw new RuntimeException("异常了，跳转到控制器通知的异常信息里");
        throw new NotFoundException(1L,"异常了，跳转到控制器通知的异常信息里");
    }
}
